...
def data_update(request):
    ...
    import utime
    # Creo una instancia asociada al sensor de luz interno
    sensorldr = ADC(Pin (36))
    sensorldr.atten(ADC.ATTN_11DB)
    # Leo el dato del sensor y ajusto para obtener la tension
    sensorldr_value = sensorldr.read()
    valor = ((4095 - sensorldr_value) / 4095) * 100
    supervalor = int (valor)
    # Ajusto para leer la luz (Seccion 3.3 de Raspberry Pi Pico Python SDK)
    # Retorno el diccionario
    return { "cpu_luz" : supervalor }  
